 ...
 * Superseded by module: games/carlaunch/route.js
...
const SETTINGS_TABLE = 'carlaunch_settings';
...
router.put('/:gameId/settings', auth, upload.fields([
  ...
  try {
    ...
    let parsedColors = e.color_options;
    if (color_options !== undefined) {
      try { parsedColors = typeof color_options === 'string' ? JSON.parse(color_options) : color_options; }
      catch { parsedColors = e.color_options; }
    }
    const fields = {
      car_make: car_make !== undefined ? car_make : (e.car_make || ''),
      car_model: car_model !== undefined ? car_model : (e.car_model || ''),
      car_year: n(car_year, e.car_year || 2024),
      car_trim: car_trim !== undefined ? car_trim : (e.car_trim || ''),
      engine_hp: n(engine_hp, e.engine_hp || 0),
      engine_torque: n(engine_torque, e.engine_torque || 0),
      engine_cylinders: n(engine_cylinders, e.engine_cylinders || 0),
      engine_displacement: n(engine_displacement, e.engine_displacement || 0),
      weight_kg: n(weight_kg, e.weight_kg || 1500),
      drivetrain: drivetrain || e.drivetrain || 'RWD',
      transmission_type: transmission_type || e.transmission_type || 'Automatic',
      gears: n(gears, e.gears || 8),
      zero_to_60: n(zero_to_60, e.zero_to_60 || 0),
      quarter_mile: n(quarter_mile, e.quarter_mile || 0),
      car_model_url: img('car_model', req.files),
      color_options: parsedColors ? JSON.stringify(parsedColors) : null,
      default_color: default_color || e.default_color || '#ef4444',
      ...
      sound_start_id: n(sound_start_id, e.sound_start_id),
      sound_shift_id: n(sound_shift_id, e.sound_shift_id),
      sound_finish_id: n(sound_finish_id, e.sound_finish_id),
