 ...
 * Superseded by module: games/catch/route.js
...
router.get('/:gameId/settings', auth, async (req, res) => {
  try {
    const [rows] = await db.query('SELECT * FROM catch_settings WHERE game_id = ?', [req.params.gameId]);
...
router.put('/:gameId/settings', auth, upload.fields([
  ...
  const {
    spawn_rate, fall_speed, max_misses, time_limit_seconds, item_type, basket_color,
    ...
    sound_catch_id, sound_miss_id, sound_gameover_id, intro_text, outro_text,
    ...
    intro_text_color, outro_text_color, thankyou_subtitle, thankyou_subtitle_color,
    submit_button_text_color, submit_button_bg_color, continue_button_text_color,
    continue_button_bg_color, start_button_text_color, start_button_bg_color,
  } = req.body;
  try {
    const [existing] = await db.query('SELECT * FROM catch_settings WHERE game_id = ?', [req.params.gameId]);
    const e = existing[0] || {};
    ...
    const fields = {
      spawn_rate: n(spawn_rate, e.spawn_rate || 1000), fall_speed: n(fall_speed, e.fall_speed || 2),
      max_misses: n(max_misses, e.max_misses || 5), time_limit_seconds: n(time_limit_seconds, e.time_limit_seconds || 60),
      item_type: item_type || e.item_type || 'emoji', basket_color: basket_color || e.basket_color || '#8B5CF6',
      ...
      sound_catch_id: sound_catch_id !== undefined && sound_catch_id !== '' ? Number(sound_catch_id) : (e.sound_catch_id || null),
      sound_miss_id: sound_miss_id !== undefined && sound_miss_id !== '' ? Number(sound_miss_id) : (e.sound_miss_id || null),
      ...
      intro_text_color: intro_text_color || e.intro_text_color || '#1a1a2e',
      ...
      thankyou_subtitle_color: thankyou_subtitle_color || e.thankyou_subtitle_color || '#888888',
      submit_button_text_color: submit_button_text_color || e.submit_button_text_color || '#ffffff',
      submit_button_bg_color: submit_button_bg_color || e.submit_button_bg_color || '#8B5CF6',
      continue_button_text_color: continue_button_text_color || e.continue_button_text_color || '#ffffff',
      continue_button_bg_color: continue_button_bg_color || e.continue_button_bg_color || '#8B5CF6',
      start_button_text_color: start_button_text_color || e.start_button_text_color || '#ffffff',
      start_button_bg_color: start_button_bg_color || e.start_button_bg_color || '#8B5CF6',
    };
    if (existing.length === 0) {
      const keys = Object.keys(fields);
      await db.query(`INSERT INTO catch_settings (game_id,${keys.join(',')}) VALUES (?,${
        keys.map(() => '?').join(',')})`, [req.params.gameId, ...Object.values(fields)]);
    } else {
      const sets = Object.keys(fields).map(k => `${k}=?`).join(',');
      await db.query(`UPDATE catch_settings SET ${sets} WHERE game_id=?`, [...Object.values(fields), req.params.gameId]);
    }
    const [updated] = await db.query('SELECT * FROM catch_settings WHERE game_id = ?', [req.params.gameId]);
