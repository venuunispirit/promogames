 ...
 * Superseded by module: games/ludo/route.js
...
router.get('/:gameId/settings', auth, async (req, res) => {
  try {
    const [rows] = await db.query('SELECT * FROM ludo_settings WHERE game_id = ?', [req.params.gameId]);
...
router.put('/:gameId/settings', auth, upload.fields([
  ...
  try {
    const [existing] = await db.query('SELECT * FROM ludo_settings WHERE game_id = ?', [req.params.gameId]);
    const e = existing[0] || {};
    ...
    if (existing.length === 0) {
      ...
      await db.query(`INSERT INTO ludo_settings (game_id,${keys.join(',')}) VALUES (?,${
        keys.map(() => '?').join(',')})`, [req.params.gameId, ...Object.values(fields)]);
    } else {
      const sets = Object.keys(fields).map(k => `${k}=?`).join(',');
      await db.query(`UPDATE ludo_settings SET ${sets} WHERE game_id=?`, [...Object.values(fields), req.params.gameId]);
    }
    const [updated] = await db.query('SELECT * FROM ludo_settings WHERE game_id = ?', [req.params.gameId]);
