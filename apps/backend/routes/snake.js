...
router.get('/:gameId/settings', auth, async (req, res) => {
  try {
    const [rows] = await db.query('SELECT * FROM snake_settings WHERE game_id = ?', [req.params.gameId]);
...
router.put('/:gameId/settings', auth, upload.fields([
  ...
  const {
    board_width, board_height, speed, snake_color, food_color, wall_mode,
  ...
  try {
    const [existing] = await db.query('SELECT * FROM snake_settings WHERE game_id = ?', [req.params.gameId]);
    const e = existing[0] || {};
    ...
    const fields = {
      board_width: n(board_width, e.board_width || 20), board_height: n(board_height, e.board_height || 20),
      speed: n(speed, e.speed || 5), snake_color: snake_color || e.snake_color || '#22c55e',
      food_color: food_color || e.food_color || '#ef4444', wall_mode: wall_mode || e.wall_mode || 'wall',
      ...
      reveal_image_url: reveal_image_url !== undefined ? reveal_image_url : (e.reveal_image_url || null),
    };
    if (existing.length === 0) {
      const keys = Object.keys(fields);
      await db.query(`INSERT INTO snake_settings (game_id,${keys.join(',')}) VALUES (?,${
        keys.map(() => '?').join(',')})`, [req.params.gameId, ...Object.values(fields)]);
    } else {
      const sets = Object.keys(fields).map(k => `${k}=?`).join(',');
      await db.query(`UPDATE snake_settings SET ${sets} WHERE game_id=?`, [...Object.values(fields), req.params.gameId]);
    }
    const [updated] = await db.query('SELECT * FROM snake_settings WHERE game_id = ?', [req.params.gameId]);
