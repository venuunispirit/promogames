...
// PUT /api/spin/:gameId/settings  (upsert)
router.put('/:gameId/settings', auth, upload.fields([
  ...
  const {
    heading_1, heading_2, description_text, spin_mode,
  ...
  try {
    ...
    if (existing.length === 0) {
      await db.query(
        `INSERT INTO spin_settings (game_id,heading_1,heading_2,description_text,spin_mode,win_message,lose_message,
         wheel_bg_color,pointer_color,center_color,center_label,bg_color,primary_color,
         bg_image_url,thankyou_bg_image_url,game_logo_url,font_family,sound_spin_id,sound_win_id,sound_lose_id)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      ...
      await db.query(
        `UPDATE spin_settings SET ${sets} WHERE game_id=?`,
...
// GET /api/spin/games/:gameId/segments
...
// POST /api/spin/games/:gameId/segments
...
// POST /api/spin/games/:gameId/segments/reorder
router.post('/games/:gameId/segments/reorder', auth, async (req, res) => {
  const { order } = req.body;
  try {
    for (const item of order) {
      await db.query('UPDATE spin_segments SET segment_order=? WHERE id=?', [item.segment_order, item.id]);
    }
