export default function BounceBuilderPage() {
  ...
  useEffect(() => {
    if (settings && id) {
      const timeout = setTimeout(() => {
        api.put(`/api/bounce/settings/${id}`, settings)
  ...
  const handleSaveSettings = async () => {
    setSaving(true);
    try {
      await api.put(`/api/bounce/settings/${id}`, settings);
  ...
  const handleAddLevel = async () => {
    ...
    try {
      const res = await api.post('/api/bounce/levels', newLevel);
  ...
  const handleUpdateLevel = async (levelId, updates) => {
    try {
      await api.put(`/api/bounce/levels/${levelId}`, updates);
  ...
  const handleDeleteLevel = async (levelId) => {
    ...
    try {
      await api.delete(`/api/bounce/levels/${levelId}`);
  ...
  const handleAddObject = async (levelId, objectData) => {
    try {
      const res = await api.post('/api/bounce/objects', { ...objectData, level_id: levelId });
  ...
  const handleUpdateObject = async (objectId, updates) => {
    try {
      await api.put(`/api/bounce/objects/${objectId}`, updates);
  ...
  const handleDeleteObject = async (objectId) => {
    ...
    try {
      await api.delete(`/api/bounce/objects/${objectId}`);
  ...
  const handleReorderObjects = async (levelId, objects) => {
    try {
      await api.post('/api/bounce/objects/reorder', { level_id: levelId, objects });
  ...
  return (
    <div className="bb-wrap" style={{ padding: '24px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: '800', color: 'var(--bb-text)', marginBottom: '4px' }}>{game.name}</h1>
          <p style={{ fontSize: '14px', color: 'var(--bb-text2)' }}>ID: {game.id}</p>
        </div>
