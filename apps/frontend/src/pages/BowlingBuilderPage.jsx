...
export default function BowlingBuilderPage() {
  const { id } = useParams()
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      const fields = ['frames','pins','difficulty',
      ...
      await api.put(`/bowling/${id}/settings`, fd)
  ...
  const renderGameplay = () => (
    <>
      <div className="gb-section">
        <div className="gb-section-title">🎳 Bowling</div>
        <div className="gb-row">
          <div className="gb-field"><label>Frames</label><input type="number" value={s('frames', 10)} onChange={e => set('frames', e.target.value)} /></div>
          <div className="gb-field"><label>Pins</label><input type="number" value={s('pins', 10)} onChange={e => set('pins', e.target.value)} /></div>
      ...
      <div className="gb-section">
        ...
        <div className="gb-row" style={{ marginTop: 8 }}>
          ...
          <ColorPicker value={s('start_button_bg_color', '#f59e0b')} onChange={v => set('start_button_bg_color', v)} label="Button BG" />
    ...
    <>
      <div className="gb-section">
        ...
        <div className="gb-row">
          ...
          <ColorPicker value={s('primary_color', '#f59e0b')} onChange={v => set('primary_color', v)} label="Primary Color" />
        </div>
  ...
  const renderSounds = () => (
    <div className="gb-section">
      ...
      <div className="gb-row">
        <SoundSelector label="Roll Sound" value={s('sound_roll_id')} onChange={v => set('sound_roll_id', v)} sounds={sounds} />
        <SoundSelector label="Strike Sound" value={s('sound_strike_id')} onChange={v => set('sound_strike_id', v)} sounds={sounds} />
        <SoundSelector label="Spare Sound" value={s('sound_spare_id')} onChange={v => set('sound_spare_id', v)} sounds={sounds} />
      </div>
        ...
        <div className="gb-h-left">
          ...
          <div>
            {editingName ? (
              ...
              <h1 className="gb-title" onClick={() => { setNameInput(game?.name || ''); setEditingName(true) }}>{game?.name || 'Bowling Game'}</h1>
            )}
            <div className="gb-subtitle">🎳 Bowling</div>
          </div>
        </div>
        ...
        <div className="gb-phone-col">
          <div className="gb-phone">
            <div style={{ padding: 16, textAlign: 'center' }}>
              ...
              <div style={{ marginTop: 8, fontSize: 12, color: '#666' }}>{s('frames', 10)} Frames · {s('pins', 10)} Pins</div>
            </div>
          </div>
        </div>
      </div>
