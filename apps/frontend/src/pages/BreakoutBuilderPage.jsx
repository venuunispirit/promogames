...
export default function BreakoutBuilderPage() {
  const { id } = useParams()
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      const fields = ['brick_rows','brick_cols','ball_speed','paddle_width','lives','difficulty',
      ...
      await api.put(`/breakout/${id}/settings`, fd)
  ...
  const handleImgUpload = (field, ref) => {
    ...
    if (file) {
      ...
      set(field, url)
      set(field.replace('_url', '_file'), file)
    }
  }
  ...
  const renderGameplay = () => (
    <>
      <div className="gb-section">
        <div className="gb-section-title">🧱 Brick Grid</div>
        <div className="gb-row">
          <div className="gb-field"><label>Brick Rows</label><input type="number" min={1} value={s('brick_rows', 5)} onChange={e => set('brick_rows', e.target.value)} /></div>
          <div className="gb-field"><label>Brick Columns</label><input type="number" min={1} value={s('brick_cols', 8)} onChange={e => set('brick_cols', e.target.value)} /></div>
        </div>
      </div>
      <div className="gb-section">
        <div className="gb-section-title">⚡ Ball &amp; Paddle</div>
        <div className="gb-row">
          <div className="gb-field"><label>Ball Speed</label><input type="number" step="0.5" min={0.5} value={s('ball_speed', 3)} onChange={e => set('ball_speed', e.target.value)} /></div>
          <div className="gb-field"><label>Paddle Width (px)</label><input type="number" min={60} max={300} value={s('paddle_width', 120)} onChange={e => set('paddle_width', e.target.value)} /></div>
        </div>
        <div className="gb-row" style={{ marginTop: 8 }}>
          <div className="gb-field"><label>Lives</label><input type="number" min={1} max={9} value={s('lives', 3)} onChange={e => set('lives', e.target.value)} /></div>
          <div className="gb-field"><label>Difficulty</label>
            <select value={s('difficulty', 'medium')} onChange={e => set('difficulty', e.target.value)}>
      ...
      <div className="gb-section">
        ...
        <div className="gb-row" style={{ marginTop: 8 }}>
          <div className="gb-field"><label>Start Button Text</label><input value={s('start_button_text', 'Start Game')} onChange={e => set('start_button_text', e.target.value)} /></div>
          <ColorPicker value={s('start_button_bg_color', '#6366f1')} onChange={v => set('start_button_bg_color', v)} label="Button BG" />
    ...
    <>
      <div className="gb-section">
        ...
        <div className="gb-row">
          ...
          <ColorPicker value={s('primary_color', '#6366f1')} onChange={v => set('primary_color', v)} label="Primary Color" />
        </div>
  ...
  const renderSounds = () => (
    <div className="gb-section">
      ...
      <div className="gb-row">
        <SoundSelector label="Paddle Hit Sound" value={s('sound_hit_id')} onChange={v => set('sound_hit_id', v)} sounds={sounds} />
        <SoundSelector label="Brick Break Sound" value={s('sound_brick_id')} onChange={v => set('sound_brick_id', v)} sounds={sounds} />
        <SoundSelector label="Life Lost Sound" value={s('sound_lose_id')} onChange={v => set('sound_lose_id', v)} sounds={sounds} />
        <SoundSelector label="Win Sound" value={s('sound_win_id')} onChange={v => set('sound_win_id', v)} sounds={sounds} />
      </div>
  ...
  const brickRows = Number(s('brick_rows', 5))
  const brickCols = Number(s('brick_cols', 8))
  const previewPaddleW = Number(s('paddle_width', 120))
  const pColors = ['#ef4444','#f59e0b','#22c55e','#3b82f6','#8b5cf6','#ec4899','#14b8a6','#f97316']
        ...
        <div className="gb-h-left">
          ...
          <div>
            {editingName ? (
              ...
              <h1 className="gb-title" onClick={() => { setNameInput(game?.name || ''); setEditingName(true) }}>{game?.name || 'Breakout Game'}</h1>
            )}
            <div className="gb-subtitle">🏓 Breakout</div>
          </div>
        </div>
        ...
        <div className="gb-phone-col">
          <div className="gb-phone">
            <div style={{ padding: 16, textAlign: 'center' }}>
              ...
              {s('heading_1') && <div style={{ fontSize: 20, fontWeight: 800, color: s('heading_1_color', '#1a1a2e') }}>{s('heading_1')}</div>}
              {s('heading_2') && <div style={{ fontSize: 14, color: s('heading_2_color', '#666'), marginTop: 4 }}>{s('heading_2')}</div>}
              {s('heading_3') && <div style={{ fontSize: 12, color: s('heading_3_color', '#777'), marginTop: 2 }}>{s('heading_3')}</div>}
              {s('description_text') && <div style={{ fontSize: 11, color: s('description_color', '#888'), marginTop: 6 }}>{s('description_text')}</div>}
              <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                {Array.from({ length: Math.min(brickRows, 5) }).map((_, ri) => (
                  <div key={ri} style={{ display: 'flex', gap: 2 }}>
                    {Array.from({ length: Math.min(brickCols, 6) }).map((_, ci) => (
                      <div key={ci} style={{ width: 20, height: 8, borderRadius: 2, background: pColors[ri % pColors.length] }} />
                    ))}
                  </div>
                ))}
                <div style={{ width: 60, height: 20, borderRadius: 4, background: '#6366f1', marginTop: 8 }} />
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#fff', marginTop: 4 }} />
              </div>
              <div style={{ marginTop: 8, fontSize: 12, color: '#666' }}>Breakout Preview</div>
            </div>
          </div>
        </div>
      </div>
