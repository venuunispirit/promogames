...
const ANIM_IN  = [
  ...
  { value:'spinIn',        label:'�dea80 Spin In' },
...
const ANIM_OUT = [
  ...
  { value:'spinOut',      label:'🪨0 Spin Out' },
...
function parseTileColors(val) {
  if (!val) return { ...DEFAULT_TILE_COLORS }
  if (typeof val === 'string') {
    try { return { ...DEFAULT_TILE_COLORS, ...JSON.parse(val) } }
    catch { return { ...DEFAULT_TILE_COLORS } }
  }
  if (typeof val === 'object') return { ...DEFAULT_TILE_COLORS, ...val }
...
/* ═══════════════════════════════════════════
   MAIN PAGE
═══════════════════════════════════════════ */
export default function Game2048BuilderPage() {
  const { id }   = useParams()
  ...
  const [game2048,      setGame2048]      = useState({
    grid_size: 4,
    target_number: 2048,
    tile_colors: JSON.stringify(DEFAULT_TILE_COLORS),
    show_timer: 0,
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      const fields = ['bg_color','primary_color',
  ...
  /* ─── 2048 Gameplay Save ─── */
  const saveGame2048 = async () => {
    setSaving(true)
    try {
      const body = {
        ...game2048,
        tile_colors: JSON.stringify(game2048.tile_colors)
      }
      await api.put(`/2048/${id}/settings`, body)
      showToast('Gameplay settings saved ✅')
    } catch (err) { showToast('Error: '+(err.response?.data?.message||err.message), 'error') }
    setSaving(false)
  }
    ...
    <div className="gb-wrap">
      ...
      <div style={{
        ...
        <div style={{ display:'flex', gap:6, alignItems:'flex-start', justifySelf:'start' }}>
          ...
          <div>
            ...
            <div style={{ fontSize:9.5, fontWeight:600, color:'var(--gb-text3)', letterSpacing:'.04em', textTransform:'uppercase', marginTop:1 }}>2048 Builder</div>
          </div>
        </div>
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'24px 0 24px 20px', display:'grid', gridTemplateColumns:'1fr 320px', gap:24, alignItems:'start' }}>
            ...
            <div>
                  ...
                  {TILE_VALUES.map(v => (
                    <div key={v} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:4 }}>
                      <div style={{
                        ...
                        background: tileColorsObj[String(v)] || DEFAULT_TILE_COLORS[String(v)],
                        display:'flex', alignItems:'center', justifyContent:'center',
                        fontWeight:800, fontSize: v >= 1000 ? 10 : 13,
                        color: v <= 4 ? '#776e65' : '#f9f6f2',
                        border:'1px solid rgba(0,0,0,0.08)',
                      }}>{v}</div>
                      <ColorPicker
                        value={tileColorsObj[String(v)] || DEFAULT_TILE_COLORS[String(v)]}
                        onChange={color => {
                          const newColors = { ...tileColorsObj, [String(v)]: color }
                          setGame2048({...game2048, tile_colors: JSON.stringify(newColors)})
                        }}
                        noPresets
                      />
                    </div>
              ...
              <div className="gb-card" style={{ marginBottom:16, padding:16 }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))', gap:12 }}>
                  ...
                  <SoundSelector label="\ud83c\udfc6 Win Sound" value={game2048.sound_win_id} onChange={v => setGame2048({...game2048, sound_win_id:v})} sounds={sounds} />
            ...
            <div>
              ...
              <div className="gb-card" style={{ marginBottom:20, padding:16 }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))', gap:12, marginBottom:16 }}>
                  <SoundSelector label="\u2b05\ufe0f Slide Sound" value={game2048.sound_slide_id} onChange={v => setGame2048({...game2048, sound_slide_id:v})} sounds={sounds} />
                  <SoundSelector label="\ud83d\udd17 Merge Sound" value={game2048.sound_merge_id} onChange={v => setGame2048({...game2048, sound_merge_id:v})} sounds={sounds} />
                  <SoundSelector label="\ud83c\udfc6 Win / Completion" value={game2048.sound_win_id} onChange={v => setGame2048({...game2048, sound_win_id:v})} sounds={sounds} />
                  <SoundSelector label="\ud83d\udc80 Lose Sound" value={game2048.sound_lose_id} onChange={v => setGame2048({...game2048, sound_lose_id:v})} sounds={sounds} />
                </div>
        ...
        <div style={{
          ...
          {/* ── Gameplay preview (2048 grid) ── */}
          {tab === 'gameplay' && (() => {
            const size = game2048.grid_size || 4
            const previewTiles = [
              { v: 2, r: 0, c: 0 },
              { v: 4, r: 0, c: 1 },
              { v: 8, r: 0, c: 2 },
            ...
            const cellSize = Math.min(40, Math.floor(220 / size))
            return (
            <div style={{
              flex:1, display:'flex', flexDirection:'column',
              background: settings.bg_image_url ? `url(${settings.bg_image_url}) center/cover` : (settings.bg_color||'#f4f4ff'),
              padding:'14px 10px',
              overflow:'auto',
              fontFamily: settings.font_family ? `'${settings.font_family}', sans-serif` : "'DM Sans', sans-serif",
            }}>
              {game2048.heading_1 && (
                <h1 style={{ fontSize:20, fontWeight:800, textAlign:'center', margin:'0 0 2px', color: game2048.heading_1_color||'#1a1a2e', lineHeight:1.2 }}>
                  {game2048.heading_1}
                </h1>
              )}
              {game2048.heading_2 && (
                <div style={{ fontSize:11, fontWeight:600, textAlign:'center', marginBottom:2, color: game2048.heading_2_color||'#64657a', lineHeight:1.3 }}>
                  {game2048.heading_2}
                </div>
              )}
              {game2048.heading_3 && (
                <div style={{ fontSize:10, fontWeight:400, textAlign:'center', marginBottom:8, color: game2048.heading_3_color||'#64657a', lineHeight:1.3 }}>
                  {game2048.heading_3}
                </div>
              )}
              <div style={{
                width: cellSize * size + 8,
                ...
                <div style={{
                  ...
                  {Array.from({ length: size * size }).map((_, idx) => {
                    const tile = previewTiles.find(t => t.r * size + t.c === idx) || null
                    const color = tile ? (tileColorsObj[String(tile.v)] || DEFAULT_TILE_COLORS[String(tile.v)]) : 'rgba(0,0,0,0.06)'
                    return (
                      <div key={idx} style={{
                        ...
                        display:'flex', alignItems:'center', justifyContent:'center',
                        fontWeight: 800,
                        fontSize: tile && tile.v >= 1000 ? 9 : tile && tile.v >= 100 ? 10 : 12,
                        color: tile && tile.v <= 4 ? '#776e65' : '#f9f6f2',
                      }}>
                        {tile ? tile.v : ''}
                      </div>
                    )
