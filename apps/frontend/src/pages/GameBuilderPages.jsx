...
const LIGHT = `.gb-wrap{--gb-bg:#f4f6fb;--gb-surface:#fff;--gb-border:#e2e6f0;--gb-primary:#f59e0b;--gb-primary-d:#d97706;--gb-primary-g:rgba(245,158,11,.15);--gb-text:#1e1e2e;--gb-text2:#64657a;--gb-text3:#9899ae;--gb-radius:12px;font-family:'DM Sans',sans-serif;background:var(--gb-bg);color:var(--gb-text);min-height:100vh}*{box-sizing:border-box}
...
function createBuilderPage(gameName, apiPath, cssColor) {
  return function BuilderPage() {
    ...
    const loadData = useCallback(() => {
      setLoading(true)
      Promise.all([api.get(`/games/${id}`), api.get(`/${apiPath}/${id}/settings`)])
        .then(([gRes, sRes]) => { setGame(gRes.data.game); setSettings(sRes.data.settings || {}) })
    ...
    return (
      <div className="gb-wrap"><style>{LIGHT}</style>
        <div style={{ display:'grid',gridTemplateColumns:'1fr auto 1fr',background:'#fff',borderBottom:'1.5px solid var(--gb-border)',padding:'10px 28px',gap:'4px 20px',alignItems:'center',position:'sticky',top:0,zIndex:50 }}>
          <div style={{ display:'flex',gap:10,alignItems:'flex-start' }}>
            ...
            <div>
              ...
              <div style={{ fontSize:9.5,fontWeight:600,color:'var(--gb-text3)',letterSpacing:'.04em',textTransform:'uppercase' }}>{gameName} Builder</div>
            </div>
          </div>
        ...
        <div style={{ maxWidth:1200,margin:'0 auto',padding:'24px 20px' }}>
          {tab === 'gameplay' && (
            <div><div className="gb-card"><div className="gb-section-title">⚙️ Game Settings</div>
              <div className="gb-row">{Object.entries(settings).filter(([k]) => !k.startsWith('heading') && !k.startsWith('bg') && !k.startsWith('primary') && !k.includes('color') && !k.includes('image') && !k.includes('text') && !k.includes('button') && !k.includes('font') && !k.includes('sound') && !k.includes('terms') && !k.includes('meta') && k !== 'id' && k !== 'game_id' && k !== 'created_at' && k !== 'updated_at').map(([k, v]) => (
                <div key={k} className="gb-fg"><span className="gb-label">{k.replace(/_/g, ' ')}</span>
                  {typeof v === 'number' ? <input type="number" value={v} onChange={e => setSettings({...settings,[k]:parseInt(e.target.value)||0})} /> : <input value={v||''} onChange={e => setSettings({...settings,[k]:e.target.value})} />}
                </div>
              ))}</div>
            </div>
            <div style={{ display:'flex',justifyContent:'flex-end' }}><button className="gb-btn gb-btn-primary" onClick={saveSettings} disabled={saving}>{saving?'⏳ Saving…':'💾 Save'}</button></div></div>
          )}

          {tab === 'visuals' && (
            <div><div className="gb-card"><div className="gb-section-title">🖼 Images</div><div className="gb-row">
            ...
            <div className="gb-card"><div className="gb-section-title">🎨 Theme</div><div className="gb-row">
              <ColorPicker label="Background" value={settings.bg_color||'#0f172a'} onChange={v => setSettings({...settings,bg_color:v})} />
              <ColorPicker label="Primary" value={settings.primary_color||cssColor} onChange={v => setSettings({...settings,primary_color:v})} />
            </div></div>
            <div style={{ display:'flex',justifyContent:'flex-end' }}><button className="gb-btn gb-btn-primary" onClick={saveSettings} disabled={saving}>{saving?'⏳ Saving…':'💾 Save'}</button></div></div>
          )}

          {tab === 'settings' && (
            <div><div className="gb-card"><div className="gb-section-title">⚙️ General</div>
