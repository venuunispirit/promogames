...
function WinOverlay({ seconds, onMenu, onPlayAgain, sceneCanvas, onComplete, redirectUrl }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(13,31,45,0.92)",backdropFilter:"blur(10px)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",zIndex:100}}>
      {phase === "lock" && (
        <div style={{transform:`scale(${scale})`,transition:"none",display:"flex",flexDirection:"column",alignItems:"center",gap:16}}>
          <div style={{fontSize:"5rem",filter:glow?"drop-shadow(0 0 30px #f5c842)":"none",transition:"filter .4s"}}>🧩</div>
          <div style={{fontSize:"1.5rem",fontWeight:900,color:glow?"#f5c842":"#fff",textShadow:glow?"0 0 20px rgba(245,200,66,0.8)":"none",transition:"all .4s",letterSpacing:1}}>
      ...
      {phase === "result" && (
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:12,animation:"slideUp .4s ease"}}>
          ...
          <div style={{fontSize:"2rem",fontWeight:900,color:"#f5c842",textShadow:"0 0 20px rgba(245,200,66,0.5)"}}>Puzzle Complete!</div>
          <div style={{color:"#7eb8cc",fontSize:"0.9rem"}}>Finished in</div>
          <div style={{fontSize:"2rem",fontWeight:800,color:"#2ecfb8"}}>{fmtTime(seconds)}</div>
          <div style={{borderRadius:16,overflow:"hidden",boxShadow:"0 8px 40px rgba(0,0,0,0.6),0 0 0 3px rgba(245,200,66,0.4)",margin:"8px 0"}}>
            <canvas ref={imgRef} style={{display:"block",width:180,height:180}} />
          </div>
          <div style={{display:"flex",gap:12,marginTop:8}}>
            <button style={{padding:"12px 28px",borderRadius:50,border:"2px solid rgba(255,255,255,0.2)",fontFamily:"inherit",fontSize:"0.98rem",fontWeight:800,cursor:"pointer",background:"transparent",color:"#7eb8cc",transition:"all .2s"}}
              onClick={onMenu}>Menu</button>
            <button style={{padding:"12px 28px",borderRadius:50,border:"none",fontFamily:"inherit",fontSize:"0.98rem",fontWeight:800,cursor:"pointer",background:"linear-gradient(135deg,#2ecfb8,#1aaa96)",color:"#0d1f2d",boxShadow:"0 4px 16px rgba(46,207,184,0.4)",transition:"all .2s"}}
...
// ─────────────────────────────────────────────────────────────────────────────
const S = {
  root:{fontFamily:"'Nunito','Segoe UI',sans-serif",background:"#1a2a3a",color:"#e8f4f8",minHeight:"100vh",userSelect:"none",overflow:"hidden"},
  menuWrap:{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#0d1f2d 0%,#1a3a4a 50%,#0d1f2d 100%)",gap:0,padding:"20px 0",overflowY:"auto"},
  menuTitle:{fontSize:"2.8rem",fontWeight:900,color:"#2ecfb8",textShadow:"0 0 30px rgba(46,207,184,0.5)",marginBottom:6,letterSpacing:-1},
  menuSub:{fontSize:"1rem",color:"#7eb8cc",marginBottom:24},
  previewWrap:{width:210,height:210,borderRadius:20,overflow:"hidden",marginBottom:24,boxShadow:"0 0 40px rgba(46,207,184,0.3),0 8px 32px rgba(0,0,0,0.5)",border:"3px solid rgba(46,207,184,0.4)",background:"#243347",position:"relative",display:"flex",alignItems:"center",justifyContent:"center"},
  diffLabel:{fontSize:"0.8rem",color:"#7eb8cc",letterSpacing:1,textTransform:"uppercase",marginBottom:12},
  diffRow:{display:"flex",gap:12,marginBottom:20},
  diffBtn:(a)=>({padding:"10px 22px",borderRadius:50,border:a?"2px solid #2ecfb8":"2px solid transparent",fontFamily:"inherit",fontSize:"0.92rem",fontWeight:700,cursor:"pointer",background:a?"#2ecfb8":"#243347",color:a?"#0d1f2d":"#7eb8cc",boxShadow:a?"0 0 16px rgba(46,207,184,0.5)":"none",transition:"all .2s"}),
  playBtn:{padding:"15px 52px",borderRadius:50,border:"none",fontFamily:"inherit",fontSize:"1.15rem",fontWeight:800,cursor:"pointer",background:"linear-gradient(135deg,#2ecfb8,#1aaa96)",color:"#0d1f2d",boxShadow:"0 6px 24px rgba(46,207,184,0.4)",transition:"all .2s"},
  // GAME layout: board in center, stats at top, tray at bottom
  gameWrap:{display:"flex",flexDirection:"column",height:"100vh",background:"linear-gradient(135deg,#0d1f2d 0%,#1a3a4a 100%)"},
  ...
  iconBtn:{background:"#243347",border:"none",borderRadius:10,color:"#7eb8cc",fontSize:"0.85rem",cursor:"pointer",padding:"6px 12px",fontFamily:"inherit",fontWeight:700,transition:"all .15s",whiteSpace:"nowrap"},
  statPill:(accent)=>({display:"flex",flexDirection:"column",alignItems:"center",background:"#243347",borderRadius:10,padding:"4px 14px",minWidth:60}),
  statVal:{fontSize:"1rem",fontWeight:900,color:"#f5c842",lineHeight:1.2},
  statLbl:{fontSize:"0.6rem",color:"#5a8a9a",textTransform:"uppercase",letterSpacing:1},
  progressBar:{flex:1,height:8,borderRadius:99,background:"rgba(255,255,255,0.1)",overflow:"hidden"},
  progressFill:(pct)=>({height:"100%",borderRadius:99,background:"linear-gradient(90deg,#2ecfb8,#5eeedd)",width:pct+"%",transition:"width .4s ease"}),
  boardArea:{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:8,overflow:"hidden"},
  trayPanel:{background:"#1e2d40",borderTop:"2px solid rgba(255,255,255,0.07)",padding:"8px 8px 8px 8px",flexShrink:0,maxHeight:160,overflowY:"auto"},
  trayTitle:{fontSize:"0.65rem",color:"#5a8a9a",letterSpacing:1,textTransform:"uppercase",marginBottom:6,textAlign:"center"},
  ...
  ghost:(x,y,size,vis)=>({position:"fixed",left:x-size/2,top:y-size/2,width:size,height:size,pointerEvents:"none",zIndex:9999,borderRadius:6,boxShadow:"0 6px 20px rgba(46,207,184,0.35)",overflow:"hidden",display:vis?"block":"none",background:"transparent"}),
};
    ...
    <div style={S.root}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ::-webkit-scrollbar{width:4px;height:4px}::-webkit-scrollbar-thumb{background:#2ecfb8;border-radius:4px}
        .tp:hover{transform:scale(1.1)!important;filter:brightness(1.1);}
        .ib:hover{background:rgba(46,207,184,0.2)!important;color:#2ecfb8!important;}
        .pb:hover{transform:translateY(-2px)!important;box-shadow:0 10px 30px rgba(46,207,184,0.5)!important;}
        .ub:hover{border-color:#2ecfb8!important;background:rgba(46,207,184,0.1)!important;}
      `}</style>
      ...
      {screen === "break" && breakData && (
        <div style={{...S.gameWrap, position:"relative"}}>
          ...
          <div style={{...S.topBar,justifyContent:"center"}}>
            <span style={{color:"#2ecfb8",fontWeight:800,fontSize:"0.9rem"}}>Get ready…</span>
          </div>
        ...
        <div style={S.gameWrap}>
          ...
          <div style={{...S.topBar, padding: isMobile ? "6px 10px" : "8px 16px", gap: isMobile ? 6 : 10}}>
            ...
            <div style={{...S.statPill(), padding: isMobile ? "3px 8px" : "4px 14px", minWidth: isMobile ? 40 : 60}}>
              <div style={{...S.statVal, fontSize: isMobile ? "0.82rem" : "1rem", color:"#2ecfb8"}}>{Math.round(pct)}%</div>
