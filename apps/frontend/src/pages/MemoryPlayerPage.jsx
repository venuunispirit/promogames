...
export default function MemoryPlayerPage({ gameData, sessionToken: initToken, onComplete }) {
  ...
  const totalPairs = uniquePairIds.size || rawTiles.length
  ...
  const handleOverlayNext = () => {
    ...
    setTimeout(() => {
      ...
      setOverlayAnimOut('')
      if (matchCount >= totalPairs) handleGameOver()
    }, 400)
  }
  ...
  const handleHint = () => {
    ...
    if (!firstCard) return
    const matchIdx = cards.findIndex((c, i) => i !== firstIdx && c.pair_id === firstCard.pair_id && !c.isMatched && !c.isFlipped)
