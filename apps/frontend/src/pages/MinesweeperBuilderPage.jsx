...
export default function MinesweeperBuilderPage() {
  const { id } = useParams()
  ...
  const loadData = useCallback(() => {
    ...
    Promise.all([
      api.get(`/games/${id}`),
      api.get(`/minesweeper/${id}/settings`),
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      const fields = ['grid_rows','grid_cols','mines','difficulty',
      ...
      await api.put(`/minesweeper/${id}/settings`, fd)
    ...
    <>
      <div className="gb-section">
        ...
        <div className="gb-row">
          <div className="gb-field"><label>Grid Rows</label><input type="number" value={s('grid_rows', 9)} onChange={e => set('grid_rows', e.target.value)} /></div>
          <div className="gb-field"><label>Grid Columns</label><input type="number" value={s('grid_cols', 9)} onChange={e => set('grid_cols', e.target.value)} /></div>
          <div className="gb-field"><label>Mines</label><input type="number" value={s('mines', 10)} onChange={e => set('mines', e.target.value)} /></div>
    ...
    <>
      <div className="gb-section">
        ...
        <div className="gb-row">
          ...
          <ColorPicker value={s('primary_color', '#22c55e')} onChange={v => set('primary_color', v)} label="Primary Color" />
        </div>
  ...
  const renderSounds = () => (
    <div className="gb-section">
      ...
      <div className="gb-row">
        <SoundSelector label="Click Sound" value={s('sound_click_id')} onChange={v => set('sound_click_id', v)} sounds={sounds} />
        <SoundSelector label="Explode Sound" value={s('sound_explode_id')} onChange={v => set('sound_explode_id', v)} sounds={sounds} />
        ...
        <div className="gb-h-left">
          ...
          <div>
            {editingName ? (
              ...
              <h1 className="gb-title" onClick={() => { setNameInput(game?.name || ''); setEditingName(true) }}>{game?.name || 'Minesweeper Game'}</h1>
            )}
            <div className="gb-subtitle">💣 Minesweeper</div>
          </div>
        </div>
        ...
        <div className="gb-phone-col">
          <div className="gb-phone">
            <div style={{ padding: 16, textAlign: 'center' }}>
              ...
              <div style={{ marginTop: 16, fontSize: 40 }}>💣</div>
              <div style={{ marginTop: 8, fontSize: 12, color: '#666' }}>{s('grid_rows', 9)}×{s('grid_cols', 9)} · {s('mines', 10)} mines</div>
            </div>
          </div>
        </div>
      </div>
