...
export default function ReactionBuilderPage() {
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      await api.put(`/reaction/${id}/settings`, fd)
      ...
      <div style={{
        ...
        <div style={{ display:'flex', gap:6, alignItems:'flex-start', justifySelf:'start' }}>
          ...
          <div>
            ...
            <div style={{ fontSize:9.5, fontWeight:600, color:'var(--gb-text3)', letterSpacing:'.04em', textTransform:'uppercase', marginTop:1 }}>Reaction Builder</div>
          </div>
        </div>
            ...
            <div>
              <div className="gb-card" style={{ marginBottom:16, padding:16 }}>
                ...
                <div className="gb-row">
                  <div className="gb-fg">
                    <span className="gb-label">Rounds</span>
                    <input type="number" min={1} max={20} value={settings.rounds??5}
                      onChange={e => setSettings({...settings, rounds: parseInt(e.target.value)||5 })} />
                  </div>
                  <div className="gb-fg">
                    <span className="gb-label">Color Change Delay (ms)</span>
                    <input type="number" min={500} max={10000} step={100} value={settings.color_change_delay??2000}
                      onChange={e => setSettings({...settings, color_change_delay: parseInt(e.target.value)||2000 })} />
                  </div>
                </div>
                <div className="gb-fg" style={{ marginTop:12 }}>
                  <label style={{ display:'flex', alignItems:'center', gap:8, fontSize:14, cursor:'pointer' }}>
                    <input type="checkbox" checked={!!settings.show_leaderboard} onChange={e => setSettings({...settings, show_leaderboard:e.target.checked?1:0})} style={{ width:16,height:16 }} />
                    Show Leaderboard
                  </label>
                </div>
              </div>
              ...
              <div className="gb-card" style={{ marginBottom:16, padding:16 }}>
                <div className="gb-section-title">Target Color</div>
                <p style={{ fontSize:12, color:'var(--gb-text2)', marginBottom:8 }}>Players must tap when they see this color</p>
                <ColorPicker value={settings.target_color||'#22c55e'} onChange={v => setSettings({...settings, target_color:v})} label="Target Color" />
              </div>
              ...
              <div className="gb-card" style={{ marginBottom:16, padding:16 }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:'8px 16px', alignItems:'end' }}>
                  <div className="gb-fg" style={{ marginBottom:0 }}>
                    <span className="gb-label">Heading 1 (title)</span>
                    <input value={settings.heading_1||''} onChange={e => setSettings({...settings, heading_1:e.target.value})} placeholder="Reaction Time" />
                  </div>
                  <ColorPicker value={settings.heading_1_color||'#1a1a2e'} onChange={v => setSettings({...settings, heading_1_color:v})} noPresets />
                  <div className="gb-fg" style={{ marginBottom:0 }}>
                    <span className="gb-label">Heading 2 (subtitle)</span>
                    <input value={settings.heading_2||''} onChange={e => setSettings({...settings, heading_2:e.target.value})} placeholder="How fast are you?" />
                  </div>
                  <ColorPicker value={settings.heading_2_color||'#666666'} onChange={v => setSettings({...settings, heading_2_color:v})} noPresets />
                  <div className="gb-fg" style={{ marginBottom:0 }}>
              ...
              <div className="gb-card" style={{ marginBottom:16, padding:16 }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                  <SoundSelector label="Correct Tap" value={settings.sound_correct_id} onChange={v => setSettings({...settings, sound_correct_id:v})} sounds={sounds} />
                  <SoundSelector label="Wrong Tap" value={settings.sound_wrong_id} onChange={v => setSettings({...settings, sound_wrong_id:v})} sounds={sounds} />
                </div>
              </div>
          ...
          {tab === 'gameplay' && (() => {
            ...
            <div style={{
              ...
              <div style={{
                ...
                <h1 style={{ fontSize:15, fontWeight:800, textAlign:'center', marginBottom:6, color: settings.heading_1_color||'#ef4444', lineHeight:1.2 }}>{settings.heading_1 || 'Reaction Time'}</h1>
                {settings.heading_2 && <div style={{ fontSize:11, fontWeight:600, textAlign:'center', marginBottom:8, color: settings.heading_2_color||'#fca5a5', lineHeight:1.3 }}>{settings.heading_2}</div>}
                <div style={{ background: settings.target_color||'#22c55e', borderRadius:12, padding:'24px 16px', textAlign:'center', marginBottom:10, cursor:'pointer', boxShadow:`0 4px 20px ${settings.target_color||'#22c55e'}44` }}>
                  <div style={{ color:'#fff', fontSize:18, fontWeight:800 }}>TAP!</div>
                  <div style={{ color:'rgba(255,255,255,0.8)', fontSize:11, marginTop:4 }}>Round 1 / {settings.rounds||5}</div>
                </div>
                <div style={{
                  width:'100%', textAlign:'center',
                  background: settings.start_button_bg_color || `linear-gradient(135deg, ${settings.primary_color||'#ef4444'}, ${(settings.primary_color||'#ef4444')}cc)`,
                  color: settings.start_button_text_color||'#fff', border:'none', borderRadius:10, padding:'10px', fontSize:13, fontWeight:700,
                  boxShadow: settings.start_button_bg_color ? '0 4px 16px rgba(0,0,0,0.12)' : `0 4px 16px ${(settings.primary_color||'#ef4444')}33`,
                  cursor:'pointer',
                }}>
                  {settings.start_button_text || 'Start Game →'}
                </div>
