...
/* ═══════════════════════════════════════════
   MAIN PAGE
═══════════════════════════════════════════ */
export default function ScrewBuilderPage() {
  const { id }   = useParams()
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      await api.put(`/screw/${id}/settings`, fd)
  ...
  const blockColors = (settings.block_colors || '#c0392b,#2980b9,#27ae60,#f39c12,#8e44ad').split(',').map(s => s.trim())
      ...
      <div style={{
        ...
        <div style={{ display:'flex', gap:6, alignItems:'flex-start', justifySelf:'start' }}>
          ...
          <div>
            ...
            <div style={{ fontSize:9.5, fontWeight:600, color:'var(--gb-text3)', letterSpacing:'.04em', textTransform:'uppercase', marginTop:1 }}>Screw & Reveal Builder</div>
          </div>
        </div>
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'24px 0 24px 20px', display:'grid', gridTemplateColumns:'1fr 320px', gap:24, alignItems:'start' }}>
        ...
        <div>
          ...
          {tab === 'gameplay' && (
            <div>
              <div className="gb-card" style={{ marginBottom:16, padding:16 }}>
                ...
                <div className="gb-row">
                  <div className="gb-fg">
                    ...
                    <input type="number" min={3} max={12} value={settings.board_rows??6}
                      onChange={e => setSettings({...settings, board_rows: parseInt(e.target.value)||6 })} />
                  </div>
                  <div className="gb-fg">
                    <span className="gb-label">Board Columns</span>
                    <input type="number" min={3} max={12} value={settings.board_cols??6}
                      onChange={e => setSettings({...settings, board_cols: parseInt(e.target.value)||6 })} />
                  </div>
                </div>
                <div className="gb-row" style={{ marginTop:12 }}>
                  <div className="gb-fg">
                    <span className="gb-label">Screws per Block</span>
                    <input type="number" min={1} max={4} value={settings.screws_per_block??2}
                      onChange={e => setSettings({...settings, screws_per_block: parseInt(e.target.value)||2 })} />
                  </div>
                  <div className="gb-fg">
                    <span className="gb-label">Difficulty</span>
                    <select value={settings.difficulty||'easy'} onChange={e => setSettings({...settings, difficulty:e.target.value})}>
                      <option value="easy">Easy (1 tap)</option>
              ...
              <div className="gb-card" style={{ marginBottom:16, padding:16 }}>
                <div className="gb-section-title">Block Colors</div>
                <p style={{ fontSize:12, color:'var(--gb-text2)', marginBottom:8 }}>Comma-separated hex colors used for board blocks</p>
                <div className="gb-row" style={{ gap:6 }}>
                  {blockColors.map((c,i) => (
                    <div key={i} style={{ display:'flex', alignItems:'center', gap:4 }}>
                      <ColorPicker value={c} onChange={v => {
                        const bc = [...blockColors]; bc[i] = v
                        setSettings({...settings, block_colors: bc.join(',')})
                      }} />
                    </div>
                  ))}
                </div>
                <div className="gb-fg" style={{ marginTop:12 }}>
                  <span className="gb-label">Colors (comma-separated)</span>
                  <input value={settings.block_colors||''} onChange={e => setSettings({...settings, block_colors:e.target.value})}
                    placeholder="#c0392b,#2980b9,#27ae60,#f39c12,#8e44ad" />
                </div>
              </div>

              <div className="gb-card" style={{ marginBottom:16, padding:16 }}>
                <div className="gb-section-title">Reveal Image</div>
                <ImageUpload label="Image revealed when all screws removed" url={settings.reveal_image_url}
                  onFile={f => { const r = new FileReader(); r.onload = ev => setSettings({...settings, reveal_image_url:ev.target.result, _revealFile:f }); r.readAsDataURL(f) }}
                  onClear={() => setSettings({...settings, reveal_image_url:'', _revealFile:null })} />
                <div className="gb-fg" style={{ marginTop:12 }}>
                  <span className="gb-label">Reveal Text</span>
                  <input value={settings.reveal_text||''} onChange={e => setSettings({...settings, reveal_text:e.target.value})} placeholder="Surprise!" />
                </div>
              </div>
              ...
              <div className="gb-card" style={{ marginBottom:16, padding:16 }}>
                ...
                {!!settings.show_timer && (
                  <div className="gb-fg">
                    ...
                    <input type="number" min={0} value={settings.time_limit_seconds||120}
                      onChange={e => setSettings({...settings, time_limit_seconds: parseInt(e.target.value)||120 })} />
                  </div>
                )}
              </div>
              ...
              <div className="gb-card" style={{ marginBottom:16, padding:16 }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:'8px 16px', alignItems:'end' }}>
                  <div className="gb-fg" style={{ marginBottom:0 }}>
                    <span className="gb-label">Heading 1 (title)</span>
                    <input value={settings.heading_1||''} onChange={e => setSettings({...settings, heading_1:e.target.value})} placeholder="Screw & Reveal" />
                  </div>
                  <ColorPicker value={settings.heading_1_color||'#1a1a2e'} onChange={v => setSettings({...settings, heading_1_color:v})} noPresets />
                  <div className="gb-fg" style={{ marginBottom:0 }}>
                    <span className="gb-label">Heading 2 (subtitle)</span>
                    <input value={settings.heading_2||''} onChange={e => setSettings({...settings, heading_2:e.target.value})} placeholder="Unscrew to reveal!" />
                  </div>
                  <ColorPicker value={settings.heading_2_color||'#666666'} onChange={v => setSettings({...settings, heading_2_color:v})} noPresets />
                  <div className="gb-fg" style={{ marginBottom:0 }}>
              ...
              <div className="gb-card" style={{ marginBottom:16, padding:16 }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                  <SoundSelector label="Screw Remove" value={settings.sound_screw_id} onChange={v => setSettings({...settings, sound_screw_id:v})} sounds={sounds} />
                  <SoundSelector label="Block Fall" value={settings.sound_fall_id} onChange={v => setSettings({...settings, sound_fall_id:v})} sounds={sounds} />
                  <SoundSelector label="Reveal" value={settings.sound_reveal_id} onChange={v => setSettings({...settings, sound_reveal_id:v})} sounds={sounds} />
                </div>
              </div>
        ...
        <div style={{
          ...
          {tab === 'gameplay' && (() => {
            ...
            const cols = parseInt(settings.board_cols) || 6
            const colors = blockColors
            const numScrews = parseInt(settings.screws_per_block) || 2
            const hasBg = settings.bg_image_url
            return (
            <div style={{
              flex:1, display:'flex', flexDirection:'column',
              ...
              <div style={{
                ...
                <h1 style={{ fontSize:15, fontWeight:800, textAlign:'center', marginBottom:6, color: settings.heading_1_color||'#1a1a2e', lineHeight:1.2 }}>{settings.heading_1 || 'Screw & Reveal'}</h1>
                ...
                <div style={{
                  ...
                  {Array.from({ length: Math.min(rows * cols, 36) }).map((_, i) => {
                    const c = colors[i % colors.length]
                    const hasScrew = i % 3 === 0
                    const tapsLeft = hasScrew ? (settings.difficulty === 'easy' ? 1 : settings.difficulty === 'medium' ? 2 : 3) : 0
                    return (
                      <div key={i} style={{
                        aspectRatio:'1', borderRadius:4,
                        background: c,
                        display:'flex', alignItems:'center', justifyContent:'center',
                        position:'relative', fontSize:8, fontWeight:700, color:'#fff',
                      }}>
                        {hasScrew && (
                ...
                <div style={{
                  ...
                  boxShadow: settings.start_button_bg_color ? '0 4px 16px rgba(0,0,0,0.12)' : `0 4px 16px ${(settings.primary_color||'#6366f1')}33`,
                  cursor:'pointer',
                }}>
                  {settings.start_button_text || 'Start Game →'}
                </div>
