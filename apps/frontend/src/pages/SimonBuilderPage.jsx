...
/* ═══════════════════════════════════════════
   MAIN PAGE
═══════════════════════════════════════════ */
export default function SimonBuilderPage() {
  const { id }   = useParams()
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      await api.put(`/simon/${id}/settings`, fd)
    ...
    <div className="gb-wrap">
      ...
      <div style={{
        ...
        <div style={{ display:'flex', gap:6, alignItems:'flex-start', justifySelf:'start' }}>
          ...
          <div>
            ...
            <div style={{ fontSize:9.5, fontWeight:600, color:'var(--gb-text3)', letterSpacing:'.04em', textTransform:'uppercase', marginTop:1 }}>Simon Builder</div>
          </div>
        </div>
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'24px 0 24px 20px', display:'grid', gridTemplateColumns:'1fr 320px', gap:24, alignItems:'start' }}>
            ...
            <div>
              <div className="gb-card" style={{ marginBottom:16, padding:16 }}>
                ...
                <div className="gb-row">
                  <div className="gb-fg">
                    <span className="gb-label">Rounds to Win</span>
                    <input type="number" min={3} max={20} value={settings.num_rounds??8}
                      onChange={e => setSettings({...settings, num_rounds: parseInt(e.target.value)||8 })} />
                  </div>
                  <div className="gb-fg">
                    <span className="gb-label">Number of Colors</span>
                    <select value={settings.num_colors??4} onChange={e => setSettings({...settings, num_colors: parseInt(e.target.value)})}>
                  ...
                  <div className="gb-fg">
                    ...
                    <select value={settings.speed||'medium'} onChange={e => setSettings({...settings, speed:e.target.value})}>
              ...
              <div className="gb-card" style={{ marginBottom:16, padding:16 }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:'8px 16px', alignItems:'end' }}>
                  <div className="gb-fg" style={{ marginBottom:0 }}>
                    <span className="gb-label">Heading 1 (title)</span>
                    <input value={settings.heading_1||''} onChange={e => setSettings({...settings, heading_1:e.target.value})} placeholder="Simon Says" />
                  </div>
                  <ColorPicker value={settings.heading_1_color||'#1a1a2e'} onChange={v => setSettings({...settings, heading_1_color:v})} noPresets />
                  <div className="gb-fg" style={{ marginBottom:0 }}>
                    <span className="gb-label">Heading 2 (subtitle)</span>
                    <input value={settings.heading_2||''} onChange={e => setSettings({...settings, heading_2:e.target.value})} placeholder="Repeat the sequence!" />
                  </div>
                  <ColorPicker value={settings.heading_2_color||'#666666'} onChange={v => setSettings({...settings, heading_2_color:v})} noPresets />
                  <div className="gb-fg" style={{ marginBottom:0 }}>
              ...
              <div className="gb-card" style={{ marginBottom:16, padding:16 }}>
                ...
                <div className="gb-fg">
                  ...
                  <textarea rows={2} value={settings.description_text||''} onChange={e => setSettings({...settings, description_text:e.target.value})} placeholder="Watch the pattern, then repeat it!" style={{ resize:'vertical' }} />
                </div>
              </div>
              ...
              <div className="gb-card" style={{ marginBottom:16, padding:16 }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                  <SoundSelector label="Correct Sequence" value={settings.sound_correct_id} onChange={v => setSettings({...settings, sound_correct_id:v})} sounds={sounds} />
                  <SoundSelector label="Wrong Sequence" value={settings.sound_wrong_id} onChange={v => setSettings({...settings, sound_wrong_id:v})} sounds={sounds} />
        ...
        <div style={{
          ...
          {/* ── Gameplay preview ── */}
          {tab === 'gameplay' && (() => {
            const numColors = parseInt(settings.num_colors) || 4
            const colors = [settings.color_1||'#ef4444', settings.color_2||'#3b82f6', settings.color_3||'#22c55e', settings.color_4||'#f59e0b', settings.color_5||'#8b5cf6', settings.color_6||'#ec4899'].slice(0, numColors)
            const hasBg = settings.bg_image_url
            return (
            <div style={{
              flex:1, display:'flex', flexDirection:'column',
              background: hasBg ? `url(${settings.bg_image_url}) center/cover` : (settings.bg_color||'#1a1a2e'),
              padding:'10px 8px',
              overflow:'auto',
              fontFamily: settings.font_family ? `'${settings.font_family}', sans-serif` : "'DM Sans', sans-serif",
            }}>
              <div style={{
                ...
                <h1 style={{ fontSize:15, fontWeight:800, textAlign:'center', marginBottom:6, color: settings.heading_1_color||'#fff', lineHeight:1.2 }}>{settings.heading_1 || 'Simon Says'}</h1>
                {settings.heading_2 && <div style={{ fontSize:11, fontWeight:600, textAlign:'center', marginBottom:8, color: settings.heading_2_color||'#aaa', lineHeight:1.3 }}>{settings.heading_2}</div>}
                <div style={{ display:'grid', gridTemplateColumns: numColors <= 4 ? '1fr 1fr' : '1fr 1fr 1fr', gap:6, marginBottom:10, maxWidth:180, margin:'0 auto 10px' }}>
                  {colors.map((c,i) => (
                    <div key={i} style={{
                      aspectRatio:'1', borderRadius: numColors <= 4 ? '50%' : '12px',
                      background: c, opacity: i === 0 ? 1 : 0.6,
                      boxShadow: i === 0 ? `0 0 20px ${c}88` : 'none',
                      transition:'all 0.3s',
                    }} />
                  ))}
                </div>
