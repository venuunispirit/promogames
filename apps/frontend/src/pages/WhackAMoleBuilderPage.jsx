...
export default function WhackAMoleBuilderPage() {
  const { id } = useParams()
  ...
  const loadData = useCallback(() => {
    ...
    Promise.all([
      api.get(`/games/${id}`),
      api.get(`/whackamole/${id}/settings`),
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      const fields = ['grid_size','mole_count','mole_time_ms','game_duration_sec','difficulty',
      ...
      await api.put(`/whackamole/${id}/settings`, fd)
    ...
    <>
      <div className="gb-section">
        ...
        <div className="gb-row">
          <div className="gb-field"><label>Grid Size</label><input type="number" min="2" max="6" value={s('grid_size', 3)} onChange={e => set('grid_size', e.target.value)} /></div>
          <div className="gb-field"><label>Mole Count</label><input type="number" min="1" value={s('mole_count', 3)} onChange={e => set('mole_count', e.target.value)} /></div>
          <div className="gb-field"><label>Mole Time (ms)</label><input type="number" min="200" step="100" value={s('mole_time_ms', 1000)} onChange={e => set('mole_time_ms', e.target.value)} /></div>
          <div className="gb-field"><label>Game Duration (sec)</label><input type="number" min="5" step="5" value={s('game_duration_sec', 30)} onChange={e => set('game_duration_sec', e.target.value)} /></div>
        </div>
        <div className="gb-row" style={{ marginTop: 8 }}>
          <div className="gb-field"><label>Difficulty</label><select value={s('difficulty', 'medium')} onChange={e => set('difficulty', e.target.value)}><option value="easy">Easy</option><option value="medium">Medium</option><option value="hard">Hard</option></select></div>
        </div>
      </div>
      <div className="gb-section">
      ...
      <div className="gb-section">
        ...
        <div className="gb-row" style={{ marginTop: 8 }}>
          <div className="gb-field"><label>Start Button Text</label><input value={s('start_button_text', 'Start Whacking')} onChange={e => set('start_button_text', e.target.value)} /></div>
  ...
  const renderSounds = () => (
    <div className="gb-section">
      ...
      <div className="gb-row">
        <SoundSelector label="Whack Sound" value={s('sound_whack_id')} onChange={v => set('sound_whack_id', v)} sounds={sounds} />
        <SoundSelector label="Miss Sound" value={s('sound_miss_id')} onChange={v => set('sound_miss_id', v)} sounds={sounds} />
  ...
  const gridSize = Number(s('grid_size', 3))
  const previewCells = Array.from({ length: gridSize * gridSize }, (_, i) => i)
        ...
        <div className="gb-h-left">
          ...
          <div>
            {editingName ? (
              ...
              <h1 className="gb-title" onClick={() => { setNameInput(game?.name || ''); setEditingName(true) }}>{game?.name || 'Whack-a-Mole'}</h1>
            )}
            <div className="gb-subtitle">🔨 Whack-a-Mole</div>
          </div>
        </div>
          ...
          <div className="gb-phone">
            <div style={{ padding: 16, textAlign: 'center' }}>
              ...
              <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#666', marginBottom: 4 }}>Whack-a-Mole Preview</div>
                <div style={{ display: 'grid', gridTemplateColumns: `repeat(${gridSize}, 1fr)`, gap: 4, maxWidth: 180, margin: '0 auto' }}>
                  {previewCells.map(i => (
                    <div key={i} style={{
                      aspectRatio: '1', borderRadius: '50%', background: '#2d2d3d', border: '2px solid #444',
                      display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: '#666'
                    }}>
                      {i % 2 === 0 ? '🐹' : ''}
                    </div>
                  ))}
              ...
              <div style={{ marginTop: 12, fontSize: 12, color: '#666' }}>{s('heading_1') || 'Whack a Mole'} Preview</div>
            </div>
          </div>
        </div>
      </div>
