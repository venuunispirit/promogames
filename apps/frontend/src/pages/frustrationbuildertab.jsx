    ...
    <div className="sb-wrap">
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'24px 20px', display:'grid', gridTemplateColumns: previewTabs.includes(activeTab) ? 'minmax(0, 1fr) 320px' : '1fr', gap:24, alignItems:'start' }}>
        ...
        <div style={{ position:'sticky', top:80, width:320, flexShrink:0 }}>
          <div style={{ width:320, height:580, borderRadius:36, border:'4px solid #1a1a2e', background:'#f4f4ff', overflow:'hidden', boxShadow:'0 12px 48px rgba(0,0,0,.18)', fontFamily: settings.font_family ? `'${settings.font_family}', sans-serif` : "'DM Sans', sans-serif", display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ flex:1, overflow:'auto', display:'flex', flexDirection:'column' }}>
              {(() => {
                ...
                return (
                  <div style={{ flex:1, display:'flex', flexDirection:'column', background: hasBg ? `url(${gameBgUrl}) center/cover` : (settings.bg_color||'#1e1b4b'), padding:'clamp(14px,4vw,20px) 12px', overflow:'auto' }}>
                    <div style={{ width:'100%', maxWidth:280, margin:'auto', background: hasBg ? 'rgba(255,255,255,0.18)' : 'rgba(255,255,255,0.93)', backdropFilter:'blur(28px)', WebkitBackdropFilter:'blur(28px)', borderRadius:22, padding:'20px 16px', boxSizing:'border-box', boxShadow: hasBg ? '0 8px 40px rgba(0,0,0,0.28), inset 0 1px 0 rgba(255,255,255,0.4)' : '0 8px 40px rgba(0,0,0,0.12)', border: hasBg ? '1px solid rgba(255,255,255,0.35)' : '1px solid rgba(255,255,255,0.85)' }}>
                      ...
                      <div style={{ fontSize:10, fontWeight:700, textAlign:'center', color:hasBg?'rgba(255,255,255,0.8)':'#888', marginBottom:8, textTransform:'uppercase', letterSpacing:'0.05em' }}>
                        Mode: {settings.game_mode || 'bubble'} | {settings.difficulty || 'medium'}
                      </div>
                      <div style={{ display:'flex', gap:8, justifyContent:'center', marginBottom:12, fontSize:10, color:hasBg?'rgba(255,255,255,0.75)':'#777' }}>
                        <span>Targets: {settings.target_count || 20}</span>
                        <span>|</span>
                        <span>Time: {settings.time_limit || 30}s</span>
                      </div>
                      ...
                      <div style={{ width:'100%', textAlign:'center', background:settings.start_button_bg_color||`linear-gradient(135deg, ${settings.primary_color||'#9333ea'}, ${(settings.primary_color||'#9333ea')}cc)`, color:settings.start_button_text_color||'#ffffff', border:'none', borderRadius:10, padding:'10px', fontSize:13, fontWeight:700, boxShadow:`0 4px 16px ${(settings.primary_color||'#9333ea')}44` }}>{settings.start_button_text||'Start Game'}</div>
                    </div>
                  </div>
                )
              })()}
          ...
          <div style={{ textAlign:'center', marginTop:10, fontSize:11, color:'var(--sb-text3)', fontWeight:600 }}>Live Preview</div>
        </div>
        )}
        ...
        <div style={{ position:'sticky', top:80, width:320, flexShrink:0 }}>
            ...
            <div style={{ flex:1, overflow:'auto', display:'flex', flexDirection:'column' }}>
              <div style={{ flex:1, display:'flex', flexDirection:'column', justifyContent:'center', background: thankyouBgUrl ? `url(${thankyouBgUrl}) center/cover` : (settings.bg_color||'#f7f7fb'), padding:'24px 14px' }}>
                <div style={{ width:'100%', maxWidth:280, margin:'0 auto', background: thankyouBgUrl ? 'rgba(255,255,255,0.86)' : '#fff', border:'1px solid rgba(255,255,255,0.7)', borderRadius:22, padding:'22px 16px', boxShadow:'0 10px 36px rgba(0,0,0,0.16)', textAlign:'center' }}>
                  ...
                  <h2 style={{ margin:'0 0 8px', fontSize:20, lineHeight:1.18, color:settings.thankyou_heading_color||'#1a1a2e', fontWeight:800 }}>{settings.thankyou_heading_text || 'You completed the game!'}</h2>
          ...
          <div style={{ textAlign:'center', marginTop:10, fontSize:11, color:'var(--sb-text3)', fontWeight:600 }}>Live Thankyou Preview</div>
        </div>
        )}
        ...
        <div style={{ position:'sticky', top:80, width:320, flexShrink:0 }}>
          ...
          <div style={{ textAlign:'center', marginTop:10, fontSize:11, color:'var(--sb-text3)', fontWeight:600 }}>Live Settings Preview</div>
        </div>
        )}
    ...
    <div class="footer">${emailTemplate.footer_text||''}</div>
          ...
          return (
            <div style={{ position:'sticky', top:80, width:320, height:580, flexShrink:0 }}>
              ...
              <div style={{ textAlign:'center', marginTop:10, fontSize:11, color:'var(--sb-text3)', fontWeight:600 }}>Live Email Preview</div>
            </div>
          )
        })()}
