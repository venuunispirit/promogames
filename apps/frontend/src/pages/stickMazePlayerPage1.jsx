...
export default function MazePlayerPage({ gameData, sessionToken, onComplete }) {
  ...
  useEffect(() => {
    ...
    const Audio_=function(){
      ...
      function whoosh(distanceCells){if(muted)return;ensure();const dur=clamp(0.12+distanceCells*0.035,0.13,0.34);const osc=ctx.createOscillator();osc.type='triangle';osc.frequency.setValueAtTime(320,now());osc.frequency.exponentialRampToValueAtTime(720,now()+dur*0.85);const filt=ctx.createBiquadFilter();filt.type='lowpass';filt.Q.value=0.5;filt.frequency.setValueAtTime(2200,now());const g=ctx.createGain();const t=now();g.gain.setValueAtTime(0.0001,t);g.gain.exponentialRampToValueAtTime(0.3,t+0.02);g.gain.exponentialRampToValueAtTime(0.0001,t+dur);osc.connect(filt);filt.connect(g);g.connect(master);osc.start();osc.stop(now()+dur+0.05)}
      function stopSpark(){}
      function bump(){if(muted)return;ensure();const osc=ctx.createOscillator();osc.type='sine';osc.frequency.setValueAtTime(220,now());osc.frequency.exponentialRampToValueAtTime(140,now()+0.09);const g=envGain(0.22,0.004,0.05,0.2,0,0.07);osc.connect(g);g.connect(master);osc.start();osc.stop(now()+0.13)}
      function collect(){if(muted)return;ensure();[784,988,1175,1568].forEach((f,i)=>{const osc=ctx.createOscillator();osc.type='triangle';const osc2=ctx.createOscillator();osc2.type='square';osc2.frequency.value=f*2;const t0=now()+i*0.055;osc.frequency.value=f;const g=ctx.createGain();g.gain.setValueAtTime(0.0001,t0);g.gain.exponentialRampToValueAtTime(0.3,t0+0.012);g.gain.exponentialRampToValueAtTime(0.0001,t0+0.26);const g2=ctx.createGain();g2.gain.setValueAtTime(0.0001,t0);g2.gain.exponentialRampToValueAtTime(0.07,t0+0.012);g2.gain.exponentialRampToValueAtTime(0.0001,t0+0.18);osc.connect(g);g.connect(master);osc2.connect(g2);g2.connect(master);osc.start(t0);osc.stop(t0+0.3);osc2.start(t0);osc2.stop(t0+0.22)})}
      function portal(){if(muted)return;ensure();[523.25,659.25,784,1046.5].forEach((f,i)=>{const osc=ctx.createOscillator();osc.type='triangle';osc.frequency.value=f;const t0=now()+i*0.09;const g=ctx.createGain();g.gain.setValueAtTime(0.0001,t0);g.gain.exponentialRampToValueAtTime(0.32,t0+0.015);g.gain.exponentialRampToValueAtTime(0.0001,t0+0.22);osc.connect(g);g.connect(master);osc.start(t0);osc.stop(t0+0.26)});const t0=now()+0.38;[1046.5,1318.5,1568].forEach(f=>{const osc=ctx.createOscillator();osc.type='sine';osc.frequency.value=f;const g=ctx.createGain();g.gain.setValueAtTime(0.0001,t0);g.gain.exponentialRampToValueAtTime(0.2,t0+0.03);g.gain.exponentialRampToValueAtTime(0.0001,t0+0.7);osc.connect(g);g.connect(master);osc.start(t0);osc.stop(t0+0.75)})}
      function click(){if(muted)return;ensure();const osc=ctx.createOscillator();osc.type='triangle';osc.frequency.value=660;const g=envGain(0.18,0.002,0.03,0.15,0,0.06);osc.connect(g);g.connect(master);osc.start();osc.stop(now()+0.1)}
      function startAmbient(){if(muted||ambientNodes)return;ensure();const chords=[[261.6,329.6,392],[392,493.9,587.3],[440,523.3,659.3],[349.2,440,523.3]];const beatDur=1.0;let chordIdx=0;const padFilter=ctx.createBiquadFilter();padFilter.type='lowpass';padFilter.frequency.value=900;padFilter.Q.value=0.7;padFilter.connect(master);function playChord(){if(muted||!ambientNodes)return;const chord=chords[chordIdx%chords.length];chordIdx++;chord.forEach(freq=>{const osc=ctx.createOscillator();const g=ctx.createGain();osc.type='sine';osc.frequency.value=freq;const t0=now();g.gain.setValueAtTime(0.0001,t0);g.gain.linearRampToValueAtTime(0.03,t0+0.12);g.gain.setValueAtTime(0.03,t0+beatDur*0.7);g.gain.linearRampToValueAtTime(0.0001,t0+beatDur*0.98);osc.connect(g);g.connect(padFilter);osc.start(t0);osc.stop(t0+beatDur+0.05)});const shimmer=ctx.createOscillator();const sg=ctx.createGain();shimmer.type='sine';shimmer.frequency.value=chord[0]*2;const t0=now();sg.gain.setValueAtTime(0.0001,t0);sg.gain.linearRampToValueAtTime(0.015,t0+0.08);sg.gain.linearRampToValueAtTime(0.0001,t0+beatDur*0.5);shimmer.connect(sg);sg.connect(padFilter);shimmer.start(t0);shimmer.stop(t0+beatDur*0.6);const bufLen=Math.floor(ctx.sampleRate*0.03);const buf=ctx.createBuffer(1,bufLen,ctx.sampleRate);const data=buf.getChannelData(0);for(let j=0;j<bufLen;j++)data[j]=(Math.random()*2-1)*(1-j/bufLen);const tick=ctx.createBufferSource();tick.buffer=buf;const tickGain=ctx.createGain();tickGain.gain.value=0.02;const tickFilter=ctx.createBiquadFilter();tickFilter.type='highpass';tickFilter.frequency.value=2000;tick.connect(tickFilter);tickFilter.connect(tickGain);tickGain.connect(master);tick.start(t0);ambientNodes.timer=setTimeout(playChord,beatDur*1000)}ambientNodes={timer:setTimeout(playChord,600)}}
    ...
    const onBeforeUnload=(e)=>{if(Game.running){e.preventDefault();e.returnValue=''}}
    window.addEventListener('beforeunload',onBeforeUnload)

    const onVisibility=()=>{if(document.hidden){Audio_.stopAmbient()}else if(!isMuted&&Game.running){Audio_.startAmbient()}}
