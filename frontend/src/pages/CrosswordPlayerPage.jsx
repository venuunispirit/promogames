...
const playKeySound = (enabled) => {
  ...
  gain.gain.exponentialRampToValueAtTime(0.0001, ctx.currentTime + 0.08) // Duration ~80ms
...
export default function CrosswordPlayerPage({ gameData, sessionToken, sessionId, onComplete }) {
  ...
  const handleUnlockHint = () => {
    if (selectedWord) {
      setUnlockedHint(selectedWord.clue_text)
    }
  }
  ...
  return (
    <div style={{ fontFamily: settings?.font_family || 'DM Sans, sans-serif', minHeight: '100vh',
      ...
      padding: '20px 16px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', position: 'relative', overflowX: 'hidden' }}>

      <div className="bg-overlay" />

      {settings?.allow_hints !== 0 && (
        <FloatingHintButton onClick={() => setHintModalOpen(true)} disabled={!selectedWord} />
      )}
      ...
      <div style={{ textAlign: 'center', marginBottom: 12, position: 'relative', zIndex: 1 }}>
        ...
        {settings?.heading_1 && <h1 style={{ margin: '0 0 4px', fontSize: 24, fontWeight: 800, color: settings?.bg_image_url ? '#fff' : '#1a1a2e', letterSpacing: '-0.02em' }}>{settings.heading_1}</h1>}
      </div>
                ...
                Array.from({ length: cols }, (_, c) => {
                  ...
                  return (
                    <div key={key} style={{ position: 'relative', width: cellSize, height: cellSize }} className={isHighlighted ? 'cell-active' : ''}>
                      {num && <span style={{ position: 'absolute', top: 2, left: 4, fontSize: cellSize * 0.22, color: 'rgba(255,255,255,0.6)', lineHeight: 1, zIndex: 2, pointerEvents: 'none', fontWeight: 600 }}>{num}</span>}
                      <input
