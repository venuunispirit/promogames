...
═══════════════════════════════════════════════ */
#home{
  width:100%;min-height:100svh;padding:120px 6% 70px;
  display:flex;align-items:center;position:relative;overflow:hidden;
  background:radial-gradient(ellipse 90% 60% at 50% -10%,rgba(146,16,246,0.22) 0%,transparent 65%),
             radial-gradient(ellipse 50% 40% at 85% 60%,rgba(97,4,151,0.14) 0%,transparent 60%),
             var(--bg);
}
...
.reward-card{padding:32px 24px;border-radius:20px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);text-align:left;transition:border-color .25s,background .25s}
.reward-card:hover{background:rgba(146,16,246,0.08);border-color:rgba(146,16,246,0.3)}
.reward-card-icon{font-size:32px;margin-bottom:16px}
.reward-card-title{font-family:var(--fh);font-size:22px;letter-spacing:1px;margin-bottom:8px}
.reward-card-desc{font-family:var(--fb);font-size:13px;color:var(--muted);line-height:1.7}
...
/* ─── LEADERBOARD MOCK DATA (shown while API loads) ─── */
const LB_MOCK = [
  ...
  return (
    <div className="hero-games">
      ...
      {!loading && games.slice(0, 5).map((game, i) => (
        <a
          key={game.id}
          href={`/play/${game.slug}/${game.client_slug}`}
          target="_blank"
        ...
        >
          ...
          {game.game_logo_url || game.bg_image_url ? (
            ...
            <div className="hg-thumb-placeholder" style={{ background: `linear-gradient(135deg,${COLORS[i % COLORS.length]}44,${COLORS[(i+2)%COLORS.length]}22)` }}>
    ...
    <>
      ...
      <div className="nav-wrap">
        <nav className="navbar">
          ...
          <ul className="nav-links">
            ...
            <li><a href="/login" className="login-link">Log in</a></li>
          </ul>
        ...
        <div className="game-grid">
          {GAME_CATEGORIES.map((g, i) => (
            <div key={g.label} className="game-cat-card" style={{ animationDelay: `${i * 70}ms` }}>
              ...
              <div className="game-cat-btn">
                Play Now <Arr size={13} />
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="lb-board">
            ...
            {LB_MOCK.map((p, i) => (
              <div key={p.name} className="lb-row">
                ...
                <div className="lb-avatar" style={{ background: `linear-gradient(135deg,${['#9210f6','#610497','#7C3AED','#4F46E5','#9210f6'][i]},${['#610497','#7C3AED','#4F46E5','#9210f6','#610497'][i]})` }}>
                  {p.initials}
                </div>
      ...
      <section id="rewards">
        <div className="rewards-inner">
          ...
          <div className="reward-tags-wrap" style={{ marginTop: 32 }}>
            {REWARD_TAGS.map(tag => (
              <span key={tag} className="reward-tag">{tag}</span>
            ))}
          </div>
          <div className="reward-cards">
            {REWARD_CARDS_DATA.map(c => (
              <div key={c.title} className="reward-card">
                <div className="reward-card-icon">{c.icon}</div>
                <div className="reward-card-title">{c.title}</div>
                <div className="reward-card-desc">{c.desc}</div>
              </div>
            ))}
          </div>
        </div>
