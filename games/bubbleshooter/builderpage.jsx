...
.gb-sticky-bar{position:sticky;top:0;z-index:40;background:rgba(244,246,551,.92);backdrop-filter:blur(8px);border-bottom:1px solid var(--gb-border);padding:10px 0;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between}
...
export default function BubbleShooterBuilderPage() {
  const { id } = useParams()
  ...
  const loadData = useCallback(() => {
    ...
    Promise.all([
      api.get(`/games/${id}`),
      api.get(`/bubbleshooter/${id}/settings`),
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      const fields = ['grid_rows','grid_cols','num_colors','difficulty',
        'heading_1','heading_2','heading_3','description_text',
        'heading_1_color','heading_2_color','heading_3_color','description_color',
        'intro_text','intro_text_color',
        'start_button_text','start_button_bg_color','start_button_text_color',
      ...
      await api.put(`/bubbleshooter/${id}/settings`, fd)
  ...
  const renderGameplay = () => (
    <>
      <div className="gb-section">
        <div className="gb-section-title">🎯 Board</div>
        <div className="gb-row">
          <div className="gb-field"><label>Grid Rows</label><input type="number" value={s('grid_rows', 8)} onChange={e => set('grid_rows', e.target.value)} /></div>
          <div className="gb-field"><label>Grid Columns</label><input type="number" value={s('grid_cols', 8)} onChange={e => set('grid_cols', e.target.value)} /></div>
          <div className="gb-field"><label>Number of Colors</label><input type="number" min={3} max={8} value={s('num_colors', 5)} onChange={e => set('num_colors', e.target.value)} /></div>
          <div className="gb-field"><label>Difficulty</label><select value={s('difficulty', 'medium')} onChange={e => set('difficulty', e.target.value)}>
  ...
  const renderSounds = () => (
    <div className="gb-section">
      ...
      <div className="gb-row">
        <SoundSelector label="Shoot Sound" value={s('sound_shoot_id')} onChange={v => set('sound_shoot_id', v)} sounds={sounds} />
        <SoundSelector label="Pop Sound" value={s('sound_pop_id')} onChange={v => set('sound_pop_id', v)} sounds={sounds} />
        <SoundSelector label="Game Over Sound" value={s('sound_gameover_id')} onChange={v => set('sound_gameover_id', v)} sounds={sounds} />
      </div>
  ...
  if (loading) return (
    <div className="gb-wrap" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh' }}>
      ...
      <div style={{ textAlign: 'center', color: 'var(--gb-text2)' }}>
        ...
        Loading Bubble Shooter builder...
  ...
  const rows = Number(s('grid_rows', 8))
  const cols = Number(s('grid_cols', 8))
  const numColors = Math.min(Number(s('num_colors', 5)), 8)
  const bubbleColors = BUBBLE_COLORS.slice(0, numColors)
        ...
        <div className="gb-h-left">
          ...
          <div>
            {editingName ? (
              ...
              <h1 className="gb-title" onClick={() => { setNameInput(game?.name || ''); setEditingName(true) }}>{game?.name || 'Bubble Shooter'}</h1>
            )}
            <div className="gb-subtitle">🏗️ Bubble Shooter</div>
          </div>
        </div>
        ...
        <div className="gb-phone-col">
          <div className="gb-phone">
            <div style={{ padding: 16, textAlign: 'center' }}>
              ...
              <div style={{ marginTop: 12, display: 'flex', justifyContent: 'center', gap: 3, flexWrap: 'wrap', maxWidth: 180, margin: '12px auto 0' }}>
                {Array.from({ length: Math.min(rows, 5) }).map((_, r) => (
                  <div key={r} style={{ display: 'flex', gap: 3, width: '100%', justifyContent: 'center', paddingLeft: r % 2 ? 10 : 0 }}>
                    {Array.from({ length: Math.min(cols, r === 0 ? 6 : 7) }).map((_, c) => (
                      <div key={c} style={{ width: 14, height: 14, borderRadius: '50%', background: bubbleColors[(r * 7 + c) % numColors] }} />
                    ))}
                  </div>
                ))}
              </div>
              <div style={{ marginTop: 12, display: 'flex', justifyContent: 'center', gap: 8 }}>
                <div style={{ width: 16, height: 16, borderRadius: '50%', background: bubbleColors[0], border: '2px solid #666' }} />
                <div style={{ fontSize: 10, color: '#666', alignSelf: 'center' }}>↓ aim & shoot</div>
              </div>
              <div style={{ marginTop: 8, fontSize: 12, color: '#666' }}>Bubble Shooter Preview</div>
            </div>
          </div>
        </div>
      </div>
