...
router.get('/:gameId/settings', auth, async (req, res) => {
  try {
    const [rows] = await db.query('SELECT * FROM bubbleshooter_settings WHERE game_id = ?', [req.params.gameId]);
...
router.put('/:gameId/settings', auth, upload.fields([
  ...
  try {
    const [existing] = await db.query('SELECT * FROM bubbleshooter_settings WHERE game_id = ?', [req.params.gameId]);
    const e = existing[0] || {};
    ...
    const fields = {
      ...
      num_colors: n(num_colors, e.num_colors || 5),
      ...
      sound_shoot_id: n(sound_shoot_id, e.sound_shoot_id),
      sound_pop_id: n(sound_pop_id, e.sound_pop_id),
    ...
    if (existing.length === 0) {
      ...
      await db.query(`INSERT INTO bubbleshooter_settings (game_id,${keys.join(',')}) VALUES (?,${
        keys.map(() => '?').join(',')})`, [req.params.gameId, ...Object.values(fields)]);
    } else {
      const sets = Object.keys(fields).map(k => `${k}=?`).join(',');
      await db.query(`UPDATE bubbleshooter_settings SET ${sets} WHERE game_id=?`, [...Object.values(fields), req.params.gameId]);
    }
    const [updated] = await db.query('SELECT * FROM bubbleshooter_settings WHERE game_id = ?', [req.params.gameId]);
