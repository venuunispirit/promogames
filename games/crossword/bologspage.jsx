...
export default function CrosswordBOLogsPage() {
  const { id } = useParams()
  ...
  const exportCSV = (bo, rows) => {
    ...
    a.href = url; a.download = `Crossword_${bo.business_name.replace(/[^a-z0-9]/gi, '_')}.csv`; a.click()
  ...
  return (
    <div style={{fontFamily:"'DM Sans',sans-serif",padding:'32px',maxWidth:1240,margin:'0 auto',color:'var(--text)'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',gap:16,flexWrap:'wrap',marginBottom:24}}>
        <div style={{display:'flex',alignItems:'center',gap:14}}>
          ...
          <div>
            ...
            <p style={{fontSize:13.5,color:'var(--text3)',marginTop:4}}>🐍 Business owner activity for this crossword game</p>
          </div>
        </div>
      ...
      {filtered.length === 0 ? (
        <div style={{textAlign:'center',padding:64,color:'var(--text3)'}}>No business owners linked to this crossword game</div>
      ) : (
          ...
          <div style={{background:'var(--surface)',borderRadius:20,width:'100%',maxWidth:1080,boxShadow:'0 30px 80px rgba(0,0,0,.3)',overflow:'hidden'}} onClick={e => e.stopPropagation()}>
            <div style={{padding:'22px 26px',background:'linear-gradient(135deg,#6366f1,#8b5cf6)',color:'#fff',display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:16,flexWrap:'wrap'}}>
              <div>
                ...
                <p style={{margin:'4px 0 0',fontSize:13,opacity:.85}}>🐍 Crossword game activity</p>
              </div>
