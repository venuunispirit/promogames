...
export default function HanoiBuilderPage() {
  const { id } = useParams()
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      const fields = ['disks','difficulty',
        ...
        'sound_move_id','sound_complete_id','sound_gameover_id',
      ...
      await api.put(`/hanoi/${id}/settings`, fd)
    ...
    <>
      <div className="gb-section">
        ...
        <div className="gb-row">
          <div className="gb-field"><label>Number of Disks</label>
            <input type="number" min={3} max={8} value={s('disks', 4)} onChange={e => set('disks', e.target.value)} />
          </div>
  ...
  const renderSounds = () => (
    <div className="gb-section">
      ...
      <div className="gb-row">
        <SoundSelector label="Move Sound" value={s('sound_move_id')} onChange={v => set('sound_move_id', v)} sounds={sounds} />
        <SoundSelector label="Complete Sound" value={s('sound_complete_id')} onChange={v => set('sound_complete_id', v)} sounds={sounds} />
  ...
  const diskCount = Number(s('disks', 4))
  const previewDisks = Array.from({ length: diskCount }, (_, i) => diskCount - i)
        ...
        <div className="gb-h-left">
          ...
          <div>
            {editingName ? (
              ...
              <h1 className="gb-title" onClick={() => { setNameInput(game?.name || ''); setEditingName(true) }}>{game?.name || 'Hanoi Tower'}</h1>
            )}
            <div className="gb-subtitle">🏗️ Hanoi Tower</div>
          </div>
        </div>
        ...
        <div className="gb-phone-col">
          <div className="gb-phone">
            <div style={{ padding: 16, textAlign: 'center' }}>
              ...
              <div style={{ marginTop: 16, display: 'flex', justifyContent: 'center', gap: 24, alignItems: 'flex-end', height: 110 }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  {previewDisks.map((d, i) => (
                    <div key={d} style={{ width: 20 + d * 8, height: 10, background: d % 2 === 0 ? '#6366f1' : '#4f46e5', borderRadius: 2, marginBottom: 1 }} />
                  ))}
                  <div style={{ width: 4, height: 40, background: '#94a3b8', marginTop: 2 }} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <div style={{ width: 4, height: 70, background: '#94a3b8' }} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <div style={{ width: 4, height: 70, background: '#94a3b8' }} />
                </div>
              ...
              <div style={{ marginTop: 8, display: 'flex', justifyContent: 'center', gap: 32 }}>
                ...
                <span style={{ fontSize: 10, color: '#888' }}>B</span>
                <span style={{ fontSize: 10, color: '#888' }}>C</span>
              </div>
              <div style={{ marginTop: 6, fontSize: 11, color: '#666' }}>{diskCount} disks · {s('difficulty', 'medium')}</div>
            </div>
          </div>
        </div>
      </div>
