...
export default function MemoryDataBOPage() {
  const { id } = useParams()
  ...
  const exportCSV = () => {
    ...
    a.href = url; a.download = `Memory_BO_Logs_${game?.name || id}.csv`; a.click()
  ...
  return (
    <div style={{fontFamily:"'DM Sans',sans-serif",padding:'32px',maxWidth:1240,margin:'0 auto',color:'var(--text)'}}>
      ...
      {entries.length === 0 ? (
        <div style={{textAlign:'center',padding:64,color:'var(--text3)'}}>No BO log entries found for this memory game</div>
      ) : (
