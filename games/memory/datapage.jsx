...
const CSS = `
@import url('${FONT_URL}');
.mem-dp *,.mem-dp *::before,.mem-dp *::after{box-sizing:border-box;margin:0;padding:0}
.mem-dp{font-family:'DM Sans',sans-serif;color:var(--text);background:var(--bg-secondary);min-height:100vh}
@keyframes memFadeUp{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:none}}
@keyframes memSpin{to{transform:rotate(360deg)}}
.mem-dp-input{width:100%;padding:10px 14px;border-radius:10px;border:1.5px solid var(--border);font-size:14px;font-family:'DM Sans',sans-serif;color:var(--text);background:var(--surface2);outline:none;transition:border-color .15s,background .15s}
.mem-dp-input:focus{border-color:var(--primary);background:var(--surface)}
.mem-dp-btn{display:inline-flex;align-items:center;gap:8px;padding:10px 20px;border-radius:10px;border:none;background:var(--primary);color:#fff;font-size:13.5px;font-family:'DM Sans',sans-serif;font-weight:600;cursor:pointer;letter-spacing:.01em;transition:background .14s,transform .1s}
.mem-dp-btn:hover{background:var(--primary-hover)}
.mem-dp-btn:disabled{opacity:.55;cursor:not-allowed}
`
...
const Ico = {
  ...
  spin: () => <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24" style={{animation:'memSpin .75s linear infinite'}}><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>,
}
...
export default function MemoryDataPage() {
  ...
  const downloadCSV = () => {
    ...
    a.href = url; a.download = `${game?.name || 'memory'}_responses.csv`; a.click()
    URL.revokeObjectURL(url)
  }

  if (loading) {
    return (
      <div className="mem-dp">
        <style>{CSS}</style>
        <div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'100vh',gap:10,color:'var(--text3)',fontSize:14}}>
          <Ico.spin/> Loading memory responses…
        </div>
      </div>
    )
  }
      ...
      <div style={{padding:'36px 40px',maxWidth:1400,margin:'0 auto'}}>
        <div style={{display:'grid',gridTemplateColumns:'auto 1fr 1.5fr auto',gap:20,alignItems:'center',marginBottom:28}}>
          ...
          <div style={{position:'relative'}}>
            ...
            <input className="mem-dp-input" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, email, score..." style={{paddingLeft:38,fontSize:13.5,height:38,width:'100%'}} />
          ...
          <button className="mem-dp-btn" onClick={downloadCSV} disabled={sorted.length === 0} style={{padding:'8px 18px',fontSize:12.5}}>
        ...
        {sorted.length === 0 ? (
          <div style={{textAlign:'center',padding:'80px 0'}}>
            ...
            <p style={{color:'var(--text3)',fontSize:14}}>
              {search ? 'Try adjusting your search terms' : 'Players who complete the memory game will appear here.'}
            </p>
          </div>
        ) : (
          <>
