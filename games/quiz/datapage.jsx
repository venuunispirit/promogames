...
const CSS = `
@import url('${FONT_URL}');
.quiz-dp *,.quiz-dp *::before,.quiz-dp *::after{box-sizing:border-box;margin:0;padding:0}
.quiz-dp{font-family:'DM Sans',sans-serif;color:var(--text);background:var(--bg-secondary);min-height:100vh}
@keyframes quizSpin{to{transform:rotate(360deg)}}
.quiz-dp-input{width:100%;padding:10px 14px;border-radius:10px;border:1.5px solid var(--border);font-size:14px;font-family:'DM Sans',sans-serif;color:var(--text);background:var(--surface2);outline:none;transition:border-color .15s,background .15s}
.quiz-dp-input:focus{border-color:var(--primary);background:var(--surface)}
.quiz-dp-btn{display:inline-flex;align-items:center;gap:8px;padding:10px 20px;border-radius:10px;border:none;background:var(--primary);color:#fff;font-size:13.5px;font-family:'DM Sans',sans-serif;font-weight:600;cursor:pointer;transition:background .14s,transform .1s}
.quiz-dp-btn:hover{background:var(--primary-hover)}
.quiz-dp-btn:disabled{opacity:.55;cursor:not-allowed}
`
...
const Ico = {
  ...
  spin: () => <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24" style={{animation:'quizSpin .75s linear infinite'}}><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>,
}
...
export default function QuizDataPage() {
  ...
  const rows = sessions.map(s => ({ session: s, playerData: parsePlayerData(s.player_data), ansMap: getAnswerMap(s.answers) }))
  ...
  const downloadCSV = () => {
    const headers = ['#', ...formKeys, 'Score', ...questions.map((q, i) => `Q${i + 1}`), ...questions.map((q, i) => `Q${i + 1} Correct?`), 'Completed At', 'Source']
    const csvRows = sorted.map((r, i) => [
      i + 1,
      ...formKeys.map(k => `"${(r.playerData[k] || '').toString().replace(/"/g, '""')}"`),
      r.session.score || 0,
      ...questions.map(q => { const a = r.ansMap[q.id]; return a ? (a.option_text || '') : '' }),
      ...questions.map(q => { const a = r.ansMap[q.id]; return a ? (a.is_correct ? 'Yes' : 'No') : '' }),
      ...
      <div style={{padding:'36px 40px',maxWidth:1400,margin:'0 auto'}}>
        <div style={{display:'grid',gridTemplateColumns:'auto 1fr 1.5fr auto',gap:20,alignItems:'center',marginBottom:28}}>
          ...
          <div style={{position:'relative'}}>
            ...
            <input className="quiz-dp-input" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, email, score..." style={{paddingLeft:38,fontSize:13.5,height:38,width:'100%'}} />
          ...
          <button className="quiz-dp-btn" onClick={downloadCSV} disabled={sorted.length === 0} style={{padding:'8px 18px',fontSize:12.5}}>
        ...
        {sorted.length === 0 ? (
          <div style={{textAlign:'center',padding:'80px 0'}}>
            ...
            <p style={{color:'var(--text3)',fontSize:14}}>
              {search ? 'Try adjusting your search terms' : 'Players who complete the quiz will appear here.'}
            </p>
          </div>
        ) : (
          <>
              ...
              <table style={{width:'100%',borderCollapse:'collapse'}}>
                <thead>
                  <tr style={{background:'var(--bg-secondary)',borderBottom:'2px solid var(--border)'}}>
                    ...
                    {questions.map((q, i) => (
                      <th key={q.id} title={q.question_text} style={{padding:'14px 16px',textAlign:'center',fontSize:11,fontWeight:700,color:'var(--text2)',textTransform:'uppercase',letterSpacing:'.08em',whiteSpace:'nowrap'}}>
                        Q{i + 1}
                      </th>
                    ))}
                  ...
                  {sorted.map((r, idx) => (
                    <tr key={r.session.id} style={{borderBottom:'1px solid var(--border-light)',background:idx % 2 === 0 ? 'var(--surface)' : 'var(--surface2)'}}>
                      ...
                      <td style={{padding:'12px 16px',fontSize:15,fontWeight:700,color:'var(--primary)',textAlign:'center'}}>
                        {r.session.score || 0}
                        {r.session.total_scoreable > 0 && <span style={{color:'var(--text3)',fontWeight:400,fontSize:12}}>/</span>}
                        {r.session.total_scoreable > 0 && <span style={{color:'var(--text3)',fontWeight:400,fontSize:12}}>{r.session.total_scoreable}</span>}
                      </td>
                      {questions.map(q => {
                        const a = r.ansMap[q.id]
                        if (!a) return <td key={q.id} style={{padding:'12px 16px',color:'var(--border-light)',textAlign:'center',fontSize:13}}>—</td>
                        const correct = a.is_correct === 1
                        return (
                          <td key={q.id} style={{padding:'12px 16px',textAlign:'center'}} title={a.option_text || ''}>
                            <div style={{display:'flex',alignItems:'center',gap:6,justifyContent:'center'}}>
                              <span style={{width:14,height:14,borderRadius:'50%',background:correct?'var(--success)':'var(--error)',flexShrink:0}} />
                              <span style={{maxWidth:100,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontSize:12.5,color:'var(--text2)'}}>
