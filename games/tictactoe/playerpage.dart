...
/// Native Tic Tac Toe player screen. Reads builder settings from [GameConfig],
/// drives the headless engine and translates its fx events into haptics.
Widget buildTicTacToePlayer(GameConfig config, GameFinished onFinished) {
  return TicTacToePlayerPage(config: config, onFinished: onFinished);
}

class TicTacToePlayerPage extends StatefulWidget {
  final GameConfig config;
  final GameFinished onFinished;
  const TicTacToePlayerPage({super.key, required this.config, required this.onFinished});

  @override
  State<TicTacToePlayerPage> createState() => _TicTacToePlayerPageState();
}

class _TicTacToePlayerPageState extends State<TicTacToePlayerPage> {
  late final TicTacToeEngine _engine;
  late Color _bgColor;
  ...
  void initState() {
    ...
    _cellColor = _hex(s['board_cell_color']?.toString()) ?? const Color(0xFFffffff);
    _bgImageUrl = s['bg_image_url']?.toString();

    _engine = TicTacToeEngine(settings: s);
                ...
                builder: (_, __) => Column(
                  ...
                  children: [
                    if (_engine.completed && _engine.winner == 'X')
                      Text('You Win!', style: TextStyle(color: _primaryColor, fontSize: 28, fontWeight: FontWeight.bold))
                    else if (_engine.completed && _engine.winner == 'O')
                      const Text('AI Wins!', style: TextStyle(color: Colors.redAccent, fontSize: 28, fontWeight: FontWeight.bold))
                    else if (_engine.completed && _engine.isDraw)
                      const Text('Draw!', style: TextStyle(color: Colors.white70, fontSize: 28, fontWeight: FontWeight.bold))
                    else
                      Text(_engine.isPlayerTurn ? 'Your turn (X)' : 'AI thinking...',
                              ...
                              child: Center(
                                child: Text(cell,
                                    style: TextStyle(
                                        ...
                                        color: cell == 'X' ? _primaryColor : Colors.white70)),
                              ),
                            ),
                          );
                        },
