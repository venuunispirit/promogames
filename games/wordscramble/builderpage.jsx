...
export default function WordScrambleBuilderPage() {
  const { id } = useParams()
  ...
  const loadData = useCallback(() => {
    ...
    Promise.all([
      api.get(`/games/${id}`),
      api.get(`/wordscramble/${id}/settings`),
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      const fields = ['rounds','time_per_word','difficulty',
      ...
      await api.put(`/wordscramble/${id}/settings`, fd)
    ...
    <>
      <div className="gb-section">
        ...
        <div className="gb-row">
          <div className="gb-field"><label>Rounds</label><input type="number" value={s('rounds', 10)} onChange={e => set('rounds', e.target.value)} /></div>
          <div className="gb-field"><label>Time Per Word (seconds)</label><input type="number" value={s('time_per_word', 30)} onChange={e => set('time_per_word', e.target.value)} /></div>
      ...
      <div className="gb-section">
        ...
        <div className="gb-row" style={{ marginTop: 8 }}>
          ...
          <ColorPicker value={s('start_button_bg_color', '#8b5cf6')} onChange={v => set('start_button_bg_color', v)} label="Button BG" />
    ...
    <>
      <div className="gb-section">
        ...
        <div className="gb-row">
          ...
          <ColorPicker value={s('primary_color', '#8b5cf6')} onChange={v => set('primary_color', v)} label="Primary Color" />
        </div>
  ...
  const renderSounds = () => (
    <div className="gb-section">
      ...
      <div className="gb-row">
        <SoundSelector label="Correct Sound" value={s('sound_correct_id')} onChange={v => set('sound_correct_id', v)} sounds={sounds} />
        <SoundSelector label="Wrong Sound" value={s('sound_wrong_id')} onChange={v => set('sound_wrong_id', v)} sounds={sounds} />
      </div>
        ...
        <div className="gb-h-left">
          ...
          <div>
            {editingName ? (
              ...
              <h1 className="gb-title" onClick={() => { setNameInput(game?.name || ''); setEditingName(true) }}>{game?.name || 'Word Scramble Game'}</h1>
            )}
            <div className="gb-subtitle">🔤 Word Scramble</div>
          </div>
        </div>
        ...
        <div className="gb-phone-col">
          <div className="gb-phone">
            <div style={{ padding: 16, textAlign: 'center' }}>
              ...
              <div style={{ marginTop: 8, fontSize: 12, color: '#666' }}>{s('rounds', 10)} rounds · {s('time_per_word', 30)}s each</div>
            </div>
          </div>
        </div>
      </div>
